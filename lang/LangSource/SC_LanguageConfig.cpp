...
bool SC_LanguageConfig::readLibraryConfigYAML(const char* fileName)
{
	...
	try {
		...
		if (doc) {
            ...
            const Node & excludeDefaultPathsNode = doc[ "excludeDefaultPaths" ];
			if (excludeDefaultPathsNode) {
				try {
                    gLanguageConfig->setExcludeDefaultPaths( excludeDefaultPathsNode.as<bool>() );
				} catch(...) {
					postfl("Warning: Cannot parse config file entry \"excludeDefaultPaths\"\n");
				}
			}

            const Node & isProject = doc[ "project" ];
			if (isProject) {
				try {
                    gLanguageConfig->setProject( isProject.as<bool>() );
				} catch(...) {
