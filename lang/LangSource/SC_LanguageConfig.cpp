...
{
	mConfigFileDirectory = boost::filesystem::absolute(boost::filesystem::path(gConfigFile)).parent_path();
}
...
void SC_LanguageConfig::setDefaultPaths()
{
	...
	if ( !( sc_IsStandAlone() || excludeDefaultPaths || project ) ) {
...
void SC_LanguageConfig::doRelativeProject(std::function<void(const char *)> func, const char *path) {
	...
	if( ( excludeDefaultPaths || project ) && (strncmp(path, confDir, strlen(confDir) ) == 0) ) {
...
void SC_LanguageConfig::doRelativeProject(std::function<void(string)> func, string path) {
	...
	if( ( excludeDefaultPaths || project ) && path.size() >= confDir.size() &&
		   std::equal( confDir.begin(), confDir.end(), path.begin() ) ) {
		boost::filesystem::path path2 = boost::filesystem::path(path);
...
bool SC_LanguageConfig::writeLibraryConfigYAML(const char* fileName)
{
	...
	out << Value << gLanguageConfig->excludeDefaultPaths;

	if(gLanguageConfig->project)
	{
	out << Key << "project";
	out << Value << true;
	}
